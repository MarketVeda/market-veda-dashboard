...
# ── VWAP ──────────────────────────────────────────────────────────────────────
def vwap(s):
    """Intraday VWAP from available candles."""
...
def sr_zones(s, window=3, min_touches=2):
    """
    Identify swing-high resistance zones and swing-low support zones.
    Returns list of {price, type:'support'|'resistance', strength, touches}
    """
    ...
    # Deduplicate (merge zones within 1%)
    merged=[]
    ...
    # Keep nearest 4 above + 4 below current price
...
def detect_patterns(s):
    """
    Detect common chart patterns from EOD OHLCV.
    Returns list of {name, direction, confidence, description}
    """
    if len(s)<10: return []
        ...
        if abs(min1_idx-min2_idx)>=5:
            ...
            if diff/lows[min1_idx]<0.03:
                patterns.append({"name":"Double Bottom","direction":"bullish",
                                "confidence":78,
                                "description":f"Two lows near ₹{round(min(lows),1)} separated by {abs(min1_idx-min2_idx)} bars. Bullish reversal pattern — neckline breakout triggers entry."})
...
def price_returns(s, ltp):
    ...
    if days>20:
        ...
        if cagr:
            cr=cagr/100
            r["t3m"]=round(ltp*(1+cr)**(3/12),2)
            r["t6m"]=round(ltp*(1+cr)**(6/12),2)
            r["t1y"]=round(ltp*(1+cr),2)
        else:
            r["t3m"]=r["t6m"]=r["t1y"]=None
    else:
        r["cagr1y"]=r["t3m"]=r["t6m"]=r["t1y"]=None
    return r
...
def compute(data):
    ...
    return {
        ...
        "ret":price_returns(s,ltp),"ee":entry_exit(s,ltp),
        "min":minervini(s),
